...
class DLL {
    ...
    public function getDeliveryTextA($delivery)
    {
        ...
        //транслит
    public function Wtranslit($text)
    { 
        return str_replace($this -> _ru, $this -> _en, $text).' '.str_replace($this -> _en, $this -> _ru, $text);
    }
    ...
    {
        if ($var)
            return trim(preg_replace("/insert|delete|update/i","", strip_tags($var)));
        else 
            return false; 
    }
}
