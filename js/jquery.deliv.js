	...
	$.fn.delivery = function(options){
       ...
       else
       {
        ...
        $('.icons-menu-print,.icons-menu-mail,a.swimprint').on(istouch,function()
        {
            ...
            {
                $('<div id="secret">Сделайте заказ</div>').dialog({modal:true,resizable:false,draggable:false,width:'250',buttons:[{text:'OK',click:function() { $( this ).remove();} }], close: function( event, ui ) {$( this ).remove();} })
        ...
        $('.menu-plus').on(istouch,function()
        {
            ...
            total+=pr
            $('.total,.swim-itogo-text,.bron-itogo').text('Итого: '+total+'грн')
        ...
        $('form').on('submit',function()
        {
            var loadp = 
            {
                ...
                beforeSubmit: function(jqForm) 
                {
                    ...
                    $('<div id="secret">Подождите, заказ обрабатывается</div>').dialog({modal:true,resizable:false,draggable:false,width:'250'})
                },
                ...
                { 
                    ...
                    if(responseText.indexOf('error')==-1)
                    {
                        ...
                        $('#secret').empty();
                        $('#secret').append('Заказ успешно оформлен<br />мы свяжемся с вами в ближайшее время').dialog({modal:true,resizable:false,draggable:false,width:'250',buttons:[{text:'OK',click:function() { $( this ).remove();} }], close: function( event, ui ) {$( this ).remove();} })
                        $('.oform').animate({height:0},1000,function()
                        {
                            $('.oform').hide();
                    ...
                    {
                        $('#secret').empty();
                    $('#secret').append(responseText.replace(/<.*?>/g, '')).dialog({modal:true,resizable:false,draggable:false,width:'250',buttons:[{text:'OK',click:function() { $( this ).remove();} }], close: function( event, ui ) {$( this ).remove();} })
        ...
        $('input[name="col"]').on('keyup',function(ev)
        {
            if(ev.keyCode!=37 && ev.keyCode!=38 && ev.keyCode!=40 && ev.keyCode!=39)
            {
              ...
              if(numbers>999){
               ...
               $('.bron-itogo').html(' <span class="text">РС‚РѕРіРѕ: </span>'+total_(numbers,nomer,days)+' РіСЂРЅ')*/
            }
        })
