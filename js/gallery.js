 ...
 var Viewer2 = {
   open: function()
   {
		...
		if(floor=='no')
		{
			...
			src_='../img/g/'+css+'/'+FOLDER+'/'+photo_menu+'.jpg';
		...
		else
		{
	        ...
	        type: "post",
	        async: false,
	        data:{id:floor,folder_site:site,folder:$('.No-3').attr('id').replace(/[^0-9]/,'')},
	        success: function(data2){
	        	//alert($('.No-3').attr('id').substr(1,1));
	        	//console.log(data2)
	            eval(data2);    
	        }    
	    });
	    ...
	    current=0;
        if (WHATFOTOOPEN) current=WHATFOTOOPEN;
		...
		if(floor=="-1")
		{
	    ...
	    else
	   	{
	   		...
	   		src_ = '/img/g/'+css+'/'+FOLDER+'/'+pictures[current];
		    var preloadImgFirst = new Image();
		    preloadImgFirst.src = src_;
	   	}
		$('.preloader').show();
        var mainImg = $("<img />",{id : "mainImg",src : src_, alt : "galery"});
        preloadImgFirst.onload=function(){galery.append('<img class="show-when-load" src="'+src_+'" style="display:none"/>');$('.show-when-load').fadeIn();$('.preloader').fadeOut();}
    ...
    {
    	 nextImg = "../img/g/"+css+'/'+FOLDER+'/'+ pictures[next];
		 prevImg = "../img/g/"+css+'/'+FOLDER+'/' + pictures[prev];
    }
	...
	$('.galery_back,.close_photos_gal').live('click touchend',function(){ Viewer2.close(); return false; $('.preloader').show();})
